  ...
  <div class="about-page">
    <section class="about-hero py-5">
      <div class="container text-center py-5">
        ...
        <p class="lead opacity-75" style="color: var(--white);">Crafting elegance since 2010</p>
      </div>
    </section>
    ...
    <section class="py-5 stats-section">
      <div ref="statsSectionRef" class="container">
        <div class="row text-center g-4">
          <div v-for="(stat, index) in stats" :key="stat.label" class="col-md-3 col-6 mb-3">
            <div class="stat-card surface p-4">
              <h3 class="display-4 fw-bold mb-2">{{ displayedStats[index] }}</h3>
              <p class="text-muted text-uppercase small mb-0">{{ stat.label }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="container">
        ...
        <div class="row g-4">
          <div v-for="member in team" :key="member.name" class="col-md-4 mb-4">
            <div class="text-center team-card surface p-4 hover-lift" style="border-radius: var(--radius-lg);">
              <div
                class="team-avatar mx-auto mb-3 d-flex align-items-center justify-content-center"
                style="width: 120px; height: 120px; border-radius: 50%;"
              >
                <i class="bi bi-person-circle text-muted" style="font-size: 3.5rem"></i>
              </div>
              <h5>{{ member.name }}</h5>
...
const startStatsAnimation = () => {
  ...
  const tick = (currentTime) => {
    ...
    if (progress < 1) { animationFrameId = window.requestAnimationFrame(tick); return; }
...
.stats-section {
  background:
    radial-gradient(circle at center, rgba(254, 181, 17, 0.08), transparent 60%);
}
